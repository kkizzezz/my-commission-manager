...
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzH5fl0x50_N8JbhNJVnltIa02LG7iwEARqMTasyKkBLQcmHA_qJHSVEw_XJTMOpjnQ/exec"; 
...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      alert("ไม่สามารถดึงข้อมูลจาก Google Sheets ได้ กรุณาตรวจสอบ URL");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const syncAction = async (action: string, payload: any) => {
    ...
    try {
      ...
      // Optionally refetch data to be sure, but for UX speed we rely on optimistic updates
    } catch (error) {
      console.error("Error syncing data:", error);
      alert("เกิดข้อผิดพลาดในการบันทึกข้อมูลลง Google Sheets");
    } finally {
      setIsSyncing(false);
    }
  };
