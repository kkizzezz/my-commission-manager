...
export default function App() {
  ...
  const loadDataFromSheet = async () => {
    ...
    try {
      ...
      // Defensive check for fetch availability
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        alert("Browser compatibility issue: " + error.message);
      } else if (error.message === 'Failed to fetch') {
        alert("ไม่สามารถเชื่อมต่อ Google Sheets ได้ (CORS Error)\n\nสาเหตุที่เป็นไปได้:\n1. ลืมกด Deploy > New Deployment ใน Apps Script\n2. ไม่ได้เลือก Who has access เป็น 'Anyone'\n3. URL ผิดพลาด");
      }
    } finally {
      setIsLoading(false);
    }
